...
function handleSystemThemeChange(e) {
  ...
  if (savedTheme === 'system') {
    ...
    document.body.classList.remove('theme-sunset', 'theme-midnight', 'theme-mint', 'dark-theme');
    if (e.matches) {
      document.body.classList.add('theme-midnight', 'dark-theme');
    } else {
...
function setupSystemThemeListener() {
  if (window.matchMedia && !systemThemeListener) {
...
function setTheme(theme) {
  ...
  } else if (theme === 'midnight' || theme === 'dark') {
    document.body.classList.remove('system-theme-listener');
    document.body.classList.add('theme-midnight', 'dark-theme');
...
function toggleTheme() {
  const current = localStorage.getItem('theme') || 'sunset';
  let next = 'sunset';
  if (current === 'sunset' || current === 'light' || current === 'system') {
    next = 'midnight';
  } else if (current === 'midnight' || current === 'dark') {
    next = 'mint';
  } else {
    next = 'sunset';
  }
  
  const buttons = document.querySelectorAll('.swad-nav-theme-btn');
  buttons.forEach(btn => {
...
function updateThemeUI() {
  ...
  // Update profile settings buttons active class
  ...
  if (lightBtn && darkBtn) {
    ...
    if (savedTheme === 'sunset' || savedTheme === 'light') {
      lightBtn.classList.add('active');
    } else if (savedTheme === 'midnight' || savedTheme === 'dark') {
  ...
  const profileToggleText = document.getElementById('profileThemeToggleText');
  const profileToggleIcon = document.getElementById('profileThemeToggleIcon');
  if (profileToggle) {
    profileToggle.classList.toggle('is-dark', isDark);
    profileToggle.setAttribute('aria-pressed', isDark ? 'true' : 'false');
  }
  if (profileToggleText) {
    profileToggleText.textContent = isDark ? 'Dark' : 'Light';
  }
  if (profileToggleIcon) {
    profileToggleIcon.setAttribute('data-lucide', isDark ? 'moon' : 'sun');
  }

  // === Pill Toggle (swadHome navbar) ===
  const pillIndicator = document.getElementById('swadPillIndicator');
  const pillLight     = document.getElementById('swadPillLight');
  const pillDark      = document.getElementById('swadPillDark');
  if (pillIndicator && pillLight && pillDark) {
    const lightW = pillLight.offsetWidth || 58;
    const darkW  = pillDark.offsetWidth  || 52;
    if (isDark) {
      pillIndicator.style.width = darkW + 'px';
      pillIndicator.style.left  = (3 + lightW) + 'px';
      pillLight.classList.remove('active');
      pillDark.classList.add('active');
    } else {
      pillIndicator.style.width = lightW + 'px';
      pillIndicator.style.left  = '3px';
  ...
  if (currentTheme === 'midnight' || currentTheme === 'dark' || isDark) {
    iconName = 'moon';
    modeLabel = 'Dark mode';
  ...
  document.querySelectorAll('.nav-mode-icon').forEach(icon => {
    icon.setAttribute('data-lucide', iconName);
  });
  document.querySelectorAll('.nav-mode-toggle').forEach(btn => {
    btn.setAttribute('aria-label', `Switch color mode. Current: ${modeLabel}`);
    btn.setAttribute('title', `Current: ${modeLabel}`);
    btn.setAttribute('aria-pressed', isDark ? 'true' : 'false');
  });

  const userIcon = document.getElementById('userThemeIcon');
  if (userIcon) {
    userIcon.setAttribute('data-lucide', iconName);
  }
...
// Self-executing theme loader on startup
(function() {
  const savedTheme = localStorage.getItem('theme') || 'system';
  const prefersDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
  if (savedTheme === 'midnight' || savedTheme === 'dark' || (savedTheme === 'system' && prefersDark)) {
    document.body.classList.add('theme-midnight', 'dark-theme');
...
// Hook into initUser to update theme buttons selection state
...
// Initial theme update for landing page on load
setupSystemThemeListener();
updateThemeUI();
// Kick off pill sizing after fonts load
window.addEventListener('load', () => { updateThemeUI(); swadBannerGoTo(0); swadBannerStart(); });
